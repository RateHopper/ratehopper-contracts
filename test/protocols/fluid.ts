...
import fluidAbi from "../../externalAbi/fluid/fluidVaultT1.json" with { type: "json" };
import fluidVaultResolverAbi from "../../externalAbi/fluid/fluidVaultResolver.json" with { type: "json" };
...
export class FluidHelper {
    ...
    async getPosition(vaultAddress: string, userAddress: string) {
        const resolver = new ethersLib.Contract(FLUID_VAULT_RESOLVER, fluidVaultResolverAbi, this.signer);
    ...
    async supply(vaultAddress: string) {
        ...
        const vault = new ethersLib.Contract(vaultAddress, fluidAbi, this.signer);
        const supplyAmount = ethersLib.parseEther(DEFAULT_SUPPLY_AMOUNT);
