...
/**
 * Script showing how to update the operator address in ProtocolRegistry through TimelockController
 *
...
async function main() {
    ...
    const NEW_OPERATOR_ADDRESS = process.env.NEW_OPERATOR_ADDRESS || "";
    ...
    if (!ethers.isAddress(NEW_OPERATOR_ADDRESS)) {
        throw new Error(`NEW_OPERATOR_ADDRESS is not a valid address: ${NEW_OPERATOR_ADDRESS}`);
    }

    console.log("Configuration:");
    console.log("- Timelock:", TIMELOCK_ADDRESS);
    console.log("- ProtocolRegistry:", PROTOCOL_REGISTRY_ADDRESS);
    console.log("- New Operator:", NEW_OPERATOR_ADDRESS);
    ...
    const currentOperator = await protocolRegistry.safeOperator();
    console.log("- Current Operator:", currentOperator);

    // Prepare operation parameters
    const target = PROTOCOL_REGISTRY_ADDRESS;
    const value = 0;
    const data = protocolRegistry.interface.encodeFunctionData("setOperator", [NEW_OPERATOR_ADDRESS]);
    ...
    if (!EXECUTE) {
        ...
        console.log("2. Run this script again with EXECUTE=true (reuse the SAME OPERATION_ID):");
        ...
        // Verify the update
        const updatedOperator = await protocolRegistry.safeOperator();
        console.log("\n=== VERIFICATION ===");
        console.log("Current operator address:", updatedOperator);
        console.log("Expected address:", NEW_OPERATOR_ADDRESS);
        console.log("Update successful:", updatedOperator.toLowerCase() === NEW_OPERATOR_ADDRESS.toLowerCase());
    }
}
