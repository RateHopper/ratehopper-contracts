 ...
 *
 * `DeployRegistryOnly`, `DeployCore`, and `DeployYieldManager` import this
 * via `m.useModule(TimelockControllerModule)`. Because Ignition derives
 * futures' IDs from `<moduleName>#<contractName>`, the timelock here always
 * has the stable ID `TimelockControllerModule#TimelockController` —
 * regardless of which top-level module called it. The journal at
 * `ignition/deployments/chain-<chainId>/` deduplicates by ID across deploy
 * runs, so calling `yarn deploy:1_core` first and `yarn deploy:2_yield_manager`
 * later reuses the same timelock address.
 *
