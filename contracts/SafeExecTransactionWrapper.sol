...
contract SafeExecTransactionWrapper {
    ...
    /**
     * @notice Executes a transaction via delegatecall and reverts if it fails
     * @dev This function is designed to be called via delegatecall from a Safe wallet
     *      It calls execTransaction on the calling Safe (msg.sender when delegatecalled becomes the Safe itself)
     * @param to Destination address of the transaction
     * @param value Ether value of the transaction
     * @param data Data payload of the transaction
     * @param operation Operation type of the transaction (Call or DelegateCall)
     ...
     * @param signatures Packed signature data ({bytes32 r}{bytes32 s}{uint8 v})
     */
    function execTransactionViaDelegateCall(
        address to,
        uint256 value,
        bytes calldata data,
        ISafe.Operation operation,
        ...
        // When called via delegatecall from Safe, address(this) is the Safe itself
        bool success = ISafe(address(this)).execTransaction{value: msg.value}(
            to,
            value,
            data,
            operation,
